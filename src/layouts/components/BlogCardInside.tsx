...
const BlogCardInside = ({ data, cat }: { data: Post; cat: string }) => {
  const { summary_length, blog_folder } = config.settings;
  const { title, image, images, author, categories, date } = data.frontmatter;
  ...
  return (
    <div className="bg-body dark:bg-darkmode-body">
      ...
      <h4 className="mb-3">
        <Link href={`/${data.path}`}>{title}</Link>
      </h4>
      <ul className="mb-4">
